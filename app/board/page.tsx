...
export default async function BoardPage({ searchParams }: BoardPageProps) {
  ...
  const [{ data: settings }, { data: donations }] = await Promise.all([
    ...
    supabase
      .from("donations")
      .select("amount, alumni_batch, full_name, team_name")
      .eq("verified", true)
      .eq("publish", true)
      .eq("status", "approved"),
  ]);
  ...
  const donorRows = [...byDonor.entries()]
    .map(([name, info]) => ({ name, total: info.total, mainBatch: pickMainBatch(info.batches), team_name: info.team_name }))
  ...
  const selectedPercent = target > 0 ? Math.min(100, Math.round((selectedTotal / target) * 100)) : 0;
    ...
    <main className="min-h-screen relative overflow-hidden bg-slate-50 font-sans p-4 md:p-8">
      ...
      <div className="relative z-10 mx-auto max-w-6xl animate-fade-up">
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="text-xs md:text-sm text-slate-500 font-medium px-3 py-1 bg-white/60 rounded-full border border-slate-200">ป้ายสัดส่วน 16:9 สำหรับแสดงหน้าจอ</div>
        </div>
        ...
        <section className="overflow-hidden rounded-3xl border border-slate-100 bg-white/80 backdrop-blur-xl shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
          <div className="px-4 py-6 md:px-8">
            <form className="mb-4 grid grid-cols-1 gap-3 md:grid-cols-4" method="GET" action="/board">
